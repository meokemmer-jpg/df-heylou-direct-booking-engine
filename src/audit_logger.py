"""Audit-Logger fuer DF-HeyLou-Direct-Booking-Engine [CRUX-MK].

HMAC-SHA256-Signed Hash-Chain. PII-redacted (DSGVO).
...
class AuditLogger:
    def __init__(
        ...
        if audit_path is None:
            audit_path = Path.home() / ".df-state" / "df-heylou-direct-booking-audit.jsonl"
        audit_path.parent.mkdir(parents=True, exist_ok=True)
        self.audit_path = audit_path
        self.secret = secret or os.environ.get(
            "DF_HEYLOU_DIRECT_BOOKING_AUDIT_SECRET", "skeleton-default-secret"
        )
